...
//
// NOTE: kessoku.go must contain ONLY the DI declarations (kessoku.Inject etc.)
// and NOT the main() function. The main() function must live in a separate
// main.go file. This is required because BUG-01 fix makes initializePackages
// fail on any type errors — a main() that calls InitApp before the band file
// is generated causes "undefined: InitApp" type errors.
...
// runGeneratedFiles accepts multiple files (relative path -> content).
// Code generation runs on kessoku.go; files other than kessoku.go are written
// AFTER code generation completes so that the type-checker does not see
// forward references to the generated injector function (e.g. InitApp) when
// loading the package during code generation.
//
// IMPORTANT: kessoku.go must NOT reference the generated injector function
// (e.g. InitApp) because the band file does not exist yet at generation time.
// Put all callers of generated functions in a separate file (e.g. main.go)
// which will be written after code generation.
func runGeneratedFiles(t *testing.T, files map[string]string, timeout time.Duration) (output string, finished bool, runErr error) {
	...
	// Write only kessoku.go (and any subdirectory files) before code generation.
	// main.go and other root-level non-kessoku files must be written afterwards
	// so that packages.Load does not see undefined references to the generated
	// injector during code generation.
	preGenFiles := make(map[string]string)
	postGenFiles := make(map[string]string)
	for name, content := range files {
		// kessoku.go and files in subdirectories are available before codegen.
		// Any other root-level .go file (e.g. main.go) is deferred until after
		// codegen so the type-checker does not see forward references.
		if name == "kessoku.go" || strings.Contains(name, "/") {
			preGenFiles[name] = content
		} else {
			postGenFiles[name] = content
		}
	}

	dir := setupRuntimeModule(t, preGenFiles)
	...
	// Now write the remaining files (e.g. main.go) that reference the generated
	// injector function.
	for name, content := range postGenFiles {
		path := filepath.Join(dir, name)
		if mkdirErr := os.MkdirAll(filepath.Dir(path), 0o750); mkdirErr != nil {
			t.Fatalf("mkdir for %s: %v", name, mkdirErr)
		}
...
// even when called with an already-cancelled context. No-error injectors accept
// a ctx parameter (async providers always inject one), but the generated body
// waits on completion channels unconditionally since cancellation cannot be
...
`,
	...
	out, finished, err := runGeneratedFiles(t, files, 120*time.Second)
	if !finished {
		t.Fatalf("generated injector timed out (deadlock):\n%s", out)
	}
	if err != nil || !strings.Contains(out, "OK") {
